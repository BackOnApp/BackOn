...
class DB {
    ...
    func loadCommitments() {
        getMyCommitments() { tasks, requests, users, error in
            guard error == nil, var tasks = tasks, var requests = requests, var users = users else {print(error!); return} //#TODO ALERT!
            let cdc = CD.controller
...
//            }
            for user in users.values {
                cdc.addPendingJob()
                user.downloadPhoto(user.photoURL){ _ in cdc.removePendingJob() }
    ...
    func refreshSignIn() {
        do {
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                let (name, surname, photoURL, phoneNumber) = self.parseUserJSON(jsonResponse: jsonResponse)
                ...
                loggedUser.downloadPhoto(photoURL) {error in
                    CD.controller.removePendingJob()
                    if error == nil {
                        print("Error while updating profile:\n",error!)
                        DispatchQueue.main.async {UIViewController.foremost.present(defaultAlert(title: "Ops... Something went wrong", message: "It seems there is a problem updating your profile.\nPlease try again later."))}
                    }
                }
            }.resume()
    ...
    func signUp(name: String, surname: String?, email: String, photoURL: URL?, completion: @escaping (ErrorString?) -> Void) {
        do {
            print("*** DB - \(#function) ***")
            var parameters: [String: String?] = ["name": name, "surname": surname, "email" : email]
            if photoURL != nil {parameters["photo"] = "\(photoURL!)"}
            let request = initJSONRequest(urlString: ServerRoutes.signUp, body: try JSONSerialization.data(withJSONObject: parameters))
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                let (name, surname, serverPhotoURL, phoneNumber) = self.parseUserJSON(jsonResponse: jsonResponse)
                let loggedUser = LoggedUser(context: self.cdc.context).populate(email: email, id: id, name: name, surname: surname, phoneNumber: phoneNumber)
                CD.controller.loggedUser = loggedUser
                CD.controller.addPendingJob()
                loggedUser.downloadPhoto(serverPhotoURL) { error in
                    CD.controller.removePendingJob()
                    return error == nil ? completion(nil) : completion("Error while downloading profilePic")
                }
            }.resume()
    ...
    func logout(completion: @escaping (ErrorString?) -> Void) {
        do {
            print("*** DB - \(#function) ***")
            let parameters: [String: String?] = ["_id": cdc.loggedUser!.id, "logoutToken" : deviceToken]
    ...
    func discover(/*completion: @escaping ([String:Discoverable]?, [String:DiscoverableUser]?, ErrorString?) -> Void*/) {
        guard let lastLocation = Geo.controller.lastLocation else { print("Location not available: no discover"); return }
        do {
            print("*** DB - \(#function) ***")
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                self.parseNeedJSONArray(jsonArray: &jsonDiscoverables, needDict: &discoverables, userDict: &users)
                let now = Date()
    ...
    func acceptDiscoverable(toAccept: Discoverable, completion: @escaping (ErrorString?) -> Void) {
        do {
            print("*** DB - \(#function) ***")
            let parameters: [String: String] = ["_id": toAccept.id, "helperID": cdc.loggedUser!.id]
    ...
    func reportTask<Content:Need>(need: Content, report: String, completion: @escaping (ErrorString?) -> Void) {
        ...
        do {
            print("*** DB - \(#function) ***")
            let parameters: [String: Any] = ["_id" : need.id, (helperToReport ? "helperReport" : "neederReport") : report]
    ...
    func updateProfile(newName: String, newSurname: String, newPhoneNumber: String, newImageEncoded: String? = nil, completion: @escaping (Int, ErrorString?) -> Void) {
        do {
            print("*** DB - \(#function) ***")
            var parameters: [String: String] = ["_id" : cdc.loggedUser!.id]
    ...
    private func parseUserJSON(jsonResponse: JSON) -> (name: String, surname: String?, photoURL: URL?, phoneNumber: String?) {
    ...
    private func parseNeedJSONArray<Content:Need, GenericUser:BaseUser>(jsonArray: inout [JSON], needDict: inout [String:Content], userDict: inout [String:GenericUser]) {
        let myID = cdc.loggedUser!.id
        for current: JSON in jsonArray {
            let lastModified = serverDateFormatter(date: current["lastModified"].stringValue)
            ...
            if user != nil {
                ...
                if userDict[userID!] == nil {
                    ...
                    userLastModified = serverDateFormatter(date: user!["lastModified"].stringValue)
                    ...
                    if type(of: userDict).Value is User.Type {
                        newUser = User(entity: User.entity, insertInto: nil).populate(email: userEmail!, id: userID!, name: userName!, surname: userSurname, phoneNumber: userPhoneNumber, lastModified: userLastModified!)
                        newUser?.photoURL = userPhotoURL
                    } else {
                        newDiscoverableUser = DiscoverableUser(id: userID!, name: userName!, surname: userSurname, email: userEmail!, phoneNumber: userPhoneNumber, lastModified: userLastModified!)
                        newDiscoverableUser?.downloadPhoto(userPhotoURL)
                    }
                } else {
                    if type(of: userDict).Value is User.Type {
            ...
            if type(of: needDict).Value is Task.Type && newUser != nil {
                let task = Task(entity: Task.entity, insertInto: nil).populate(id: id, needer: newUser!, title: title, descr: descr, latitude: latitude, longitude: longitude, date: date, lastModified: lastModified)
                ...
                let request = Request(entity: Request.entity, insertInto: nil).populate(id: id, helper: newUser, title: title, descr: descr, latitude: latitude, longitude: longitude, date: date, lastModified: lastModified)
                ...
                let discoverable = Discoverable(needer: newDiscoverableUser!, title: title, date: date, latitude: latitude, longitude: longitude, id: id, lastModified: lastModified)
